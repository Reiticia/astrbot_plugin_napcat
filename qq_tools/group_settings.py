...
async def set_group_add_option(client, group_id: int, option: str) -> dict:
    await client.call_action('set_group_add_request', group_id=group_id, flag=option)
    return {'ok': True, 'detail': '加群方式已更新'}
...
async def get_group_members_info(client, group_id: int) -> dict:
...
async def get_group_honor_info(client, group_id: int, type: str = 'all') -> dict:
    data = await client.call_action('get_group_honor_info', group_id=group_id, type=type)
    return {'ok': True, 'result': truncate(json.dumps(data, ensure_ascii=False, indent=2))}


async def get_group_shut_list(client, group_id: int) -> dict:
    ...
    return {'ok': True, 'result': truncate(
        json.dumps(shut, ensure_ascii=False, indent=2)) if shut else '没有被禁言的成员'}
